...
| M9 | Hardening: self-observability + SLO, runbooks, rollback drill | 🔨 M9a self-observability done: dashboard, 5 `ops` alarms, `docs/slo.md` + `slo-reporter`, `CapReached` ([#26](https://github.com/KucherenkoSerhiy/oncall-triage/issues/26)); M9b done: weekly `known-issues-export` to S3, `bankops known-issues import`, `bankops replay-dlq` ([#27](https://github.com/KucherenkoSerhiy/oncall-triage/issues/27)); DNSSEC, OIDC subjects, runbooks and the rollback drill remain ([#28](https://github.com/KucherenkoSerhiy/oncall-triage/issues/28)–[#30](https://github.com/KucherenkoSerhiy/oncall-triage/issues/30)) |
